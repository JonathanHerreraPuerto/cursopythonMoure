...
class Olympics:
    ...
    def resgister_event(self):
        ...
        if event in self.events:
            ...
            print(f"Evento {event} Ha sido registrado exitosamente.")
            
            
    def register_participant(self):
        
        if not self.events:
            print("No hay eventos resgistrados. Por favor, registra uno primero.")
            return
        ...
        if event_choice >=0 and event_choice < len(self.events):
            ...
            print ("Selección de evento deportivo inválido. El participánte no se ha registrado.")
    ...
    def simulate_events(self):
        ...
        for event in self.events:
            ...
            if len(self.participants[event]) < 3 :
                print(f"No hay suficientes participantes para simular el evento{event}(mínimo 3).")
                continue
            ...
            print(f"oro: {silver.name} ({silver.country})")
            print(f"oro: {bronze.name} ({bronze.country})")
    ...
    def show_report(self):
        ...
        if self.country_results:
            ...
            for country, medals in sorted (self.country_results.items(), key=lambda x:(
                ...
                print(f"{country}:Oro{medals['gold']},{country}:Plata{medals['silver']},{country}:Bronze{medals['bronze']}")
